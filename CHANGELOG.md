...
- **Warning: throw non-Error object**: BinaryManager.ts 	hrow lastError now checks instanceof Error before throwing
- **Warning: unsafe ny from JSON.parse**: cliConfigReader.ts uses s Record<string, unknown> type assertion instead of variable type annotation
- **Warning: unsafe construction/return in registerView**: main.ts (leaf: WorkspaceLeaf) => typed callback resolves unsafely-typed leaf parameter and unused WorkspaceLeaf import simultaneously
- **Warning: unnecessary double assertion**: KiloCodeChatRuntime.ts removed redundant s string after non-null assertion

### Changed

- **Recommendation: display deprecation**: Migrated SettingsTab to implement getSettingDefinitions() for Obsidian 1.13.0+ declarative settings API; display() retained as fallback for older versions
- **minAppVersion bump**: Updated to 1.13.0 for getSettingDefinitions() support
- **Obsidian types**: Upgraded local type definitions to 1.13.0
# Changelog
...
- **Error: SettingsTab 标题违规**: `'KiloCode Settings'` �?`'Configuration'`（移�?settings/插件名）；`'General'` �?`'Basic'`
- **Warning: unsafe `any`**: `KiloCodeChatRuntime.ts` 引入 `KiloSession`/`KiloClientInternals`/`StreamEventPart` 接口，消除所�?`(something as any)` 访问；`cliConfigReader.ts`/`BinaryManager.ts`/`main.ts` 相应修复
- **Warning: Promise 未处�?*: `ApprovalManager.ts` `.catch()` 补全；`ImageContext.ts`/`main.ts` 多处 `void` 标记
- **Warning: `document` �?`activeDocument`**: `MessageRenderer.ts`/`ImageContext.ts`/`FileAttachmentContext.ts` �?7 处替�?
...
- **Warning: unsafe return/assertion**: `cliConfigReader.ts` JSON.parse 结果加类型断言；`main.ts` `loadData()` �?`Partial<>` 约束
...
- **CI 类型错误**: `cliConfigReader.ts` �?`unknown` �?`string | undefined` 断言；`types.ts` re-export `Message`；`MarkdownRenderer.render` 补全 5 参数签名；`VirtualScroller` 回调显式 `Message` 类型；测�?mock 同步更新

## [0.9.5] - 2026-06-11

### Security

- **Release Attestation**: �?`main.js` / `styles.css` 添加 GitHub Artifact Attestation 签名，满�?Obsidian 商城审核要求
- **移除 `os` 模块依赖**：`PlatformDetector.ts` / `cliConfigReader.ts` 不再 import `os` 模块，消�?系统身份信息读取"警告（`os.arch()` �?`process.arch`，`os.platform()` �?`process.platform`，`os.homedir()` �?env vars�?

### Changed

- **Error: 静态样式赋�?*: `CustomInstructionModal` / `ImageContext` / `SettingsTab` / `VirtualScroller` 中使�?`setCssStyles()` 替代 `.style.xxx = yyy`，满�?`no-static-styles-assignment` 要求
- **Error: HTML 标题元素**: `SettingsTab` 全部 8 �?`<h2>/<h3>` 替换�?`Setting.setHeading()`
- **Warning: 空块语句**: 所有空 `catch {}` 添加注释，消�?no-empty 警告
- **Warning: unsafe any 类型**: `cliConfigReader.ts` / `MCPToolAdapter.ts` / `MCPManager.ts` / `VirtualScroller.ts` �?`any` 替换�?`Record<string, unknown>` 或泛�?
...
- **Warning: 硬编码路�?*: `'.obsidian'` 替换�?`this.app.vault.configDir`
- **其他清理**: 未使用变�?导入移除；`require('path')` �?`import * as pathModule`；`'LGTM' | string` �?`string`；多余类型断言清理
...
- **versions.json**: 版本映射文件，为提交 Obsidian 社区插件商城做准�?
- manifest.json 版本号更新至 0.9.3

### Changed

- **README 产品定位重构**: Hero 定位�?你的 Obsidian 知识库就是上下文"改为"An Obsidian plugin that turns your vault into persistent memory for KiloCode"。新�?问题 (The Problem)"�?什么是记忆系统"�?变化对比 (Before/After)"三个章节，将产品叙事�?双向桥梁/聊天面板"升级�?KiloCode 的长期记忆系�?
- **manifest.json**: 插件描述更新为聚�?persistent memory 价�?
- **package.json**: 描述和关键词更新，新�?persistent-memory、ai-memory 标签
...
- **Slash Command 完整实现**: CommandPalette �?stub 实现为完整命令面板，支持二级子菜单（`/skill` 展示技能列表、`/model` 列出 CLI 模型支持自定义输入、`/mode` 切换 plan/code/ask、`/compact` 压缩会话、`/clear` 新建会话），键盘导航（↑↓EnterEsc），子菜单后退/关闭按钮（`src/features/commands/CommandPalette.ts`, `SlashCommand.ts`, `KiloCodeView.ts`�?
- **@mention 完整实现**: MentionDropdown 新增键盘导航（↑↓EnterEsc）与选中�?CSS 样式，MentionService 新增前缀匹配优先+子串回退搜索策略、递归文件夹搜索、MentionContext 接口，类型标题使�?i18n 翻译键（`src/features/mention/MentionDropdown.ts`, `MentionService.ts`�?
- **Custom Instruction Modal**: `#` 工具栏按钮从 stub 实现为完整的 CustomInstructionModal，支持编�?保存/Apply 自定义指令，持久化至插件设置（`src/features/chat/ui/CustomInstructionModal.ts`, `KiloCodeView.ts`�?
- **File Attachment 完整实现**: `attachFile()` �?stub 实现�?FileAttachmentContext，支持系统文件选择器，文本文件内容自动注入到消息，预览芯片渲染（`src/features/chat/ui/FileAttachmentContext.ts`, `KiloCodeView.ts`�?
- **Plugin 生命周期增强**: KiloCodePlugin 新增 runtime 集合管理、后台预热（autoStart 启用时异步预启动 CLI）、技能热重载监听器（SkillWatcher）、退出处理器（process.on('exit') 兜底清理子进程）、onunload 统一清理所�?runtime（`src/main.ts`�?
- **技能管理命�?*: 新增 "List available skills" �?"Install skill: <name>" 命令，自动为每个内置技能生成独立安装命令（`src/main.ts`�?
...
- **图片附件功能**: `capabilities.ts` �?`supportsImageAttachments` 设为 false，移�?README 中图片附件的功能宣称

### Changed

- **defaultSettings.ts**: 新增 `customInstructions: ''` 字段
- **项目清理**: 删除 docs/、findings.md、git_orig.ts、git_original.ts、progress.md、task_plan.md 等个人临时文件；更新 .gitignore 移除 styles.css（源文件应被跟踪）、新�?docs/ �?*.orig* 模式

## [0.9.1] - 2026-05-27

### Added

- **自定义指令编辑器**: `#` 工具栏按钮已从存 stub 实现为完整的浮动弹窗（`CustomInstructionModal`），支持编辑自定义指令文本并应用到当前对�?session。文本自动保存至插件设置（data.json），点击 Apply 后注�?system prompt，切�?session 需重新 Apply（`src/features/chat/ui/CustomInstructionModal.ts`, `KiloCodeChatRuntime.ts`, `KiloCodeView.ts`�?

### Removed

- **图片附件功能**: 移除图片附件（粘�?拖拽/按钮）功能。该功能通过 `FilePartInput` �?CLI 发送图片但 AI 模型无法识别，属于不完整实现。移�?`ImageContext` �?`KiloCodeView` 中的所有引用、工具栏 "Attach image" 按钮、粘�?拖拽图片事件处理、`MessageContext.images` 字段、Runtime 中的 `FilePartInput` 图片 part 构建逻辑（`KiloCodeView.ts`, `KiloCodeChatRuntime.ts`, `types.ts`, `capabilities.ts`�?

### Fixed

- **图片附件未发�?*: 修复图片附加功能实际未将图片发送给 AI 的问题。根因：`KiloCodeView` 获取了图片但未传递给 `runtime.sendMessage()` �?`userMessage`；`KiloCodeChatRuntime.sendMessage()` 仅发送文�?part，未包含图片 `FilePartInput`。修复：`MessageContext` 新增 `images` 字段，Runtime 将图片作�?`{ type: 'file', mime, filename, url }` part 发送，View 层将 images 传入 runtime 并保存到消息历史（`src/core/providers/types.ts`, `KiloCodeChatRuntime.ts`, `KiloCodeView.ts`�?

## [0.9.0] - 2026-05-26

### Added

- **文件附件功能**: `FileAttachmentContext` 支持通过系统文件选择器附加任意文件，文本文件内容随消息发送给 AI，二进制文件标注文件名和大小。输入区域显示文件预览芯片（名称+大小+删除按钮）。工具栏 "Attach vault file" 图标修复�?📎�?Attach image" 修复�?🖼️；"Current note" 修复�?📝（`src/features/chat/ui/FileAttachmentContext.ts`�?
- **@mention 交互系统**: 实现完整�?@mention 流程 �?工具�?`@` 按钮和输入框 `@` 字符触发搜索下拉，支持文�?文件�?MCP 服务�?subagent 四种类型搜索。前缀匹配优先，无结果回退子串匹配。键盘导�?↑↓EnterEsc（`src/features/mention/@Parser.ts`, `MentionService.ts`, `MentionDropdown.ts`�?
...
- **CLI 配置读取�?*: `cliConfigReader.ts` 支持�?`kilo.jsonc`/`kilo.json`/`config.json` 读取 agent 模型 ID 列表，JSONC 解析器支持注释和尾逗号
- **技能注入系�?*: `KiloCodeChatRuntime` 在每次发送消息前自动加载 vault `.kilo/skills/` 中的技能（包含 `kilocode-core` 核心技能和专业技能），拼接到消息前作为系统上下文，并附加 `QUESTION_PROTOCOL` 协议提示（`SkillLoader.ts`, `SkillCatalog.ts`, `SkillWatcher.ts`, `prompts.ts`�?
- **EventBuffer 事件缓冲**: 流式事件环形缓冲（最�?500 条），支持二分查�?`getSince()`、`replay()` 回放、`clear()` 清理。用于跨标签页切换时恢复流式渲染状态（`EventBuffer.ts`�?
- **�?Runtime 架构**: 每个 Tab 独立持有 `ChatRuntime` 实例，`Tab.runtime` 字段代替全局 `InputController`。`TabManager.disposeAllRuntimes()` 统一清理。标签关闭时自动 stop 对应进程（`Tab.ts`, `TabManager.ts`�?
- **后台预热**: `warmupRuntime()` 在视图打开时异步预启动 CLI 进程（仅二进制已缓存时），加速首次发送消息的响应速度
- **EventBuffer 恢复**: 切换到有缓冲事件的标签时，自动从 runtime �?`eventBuffer.replay()` 重建流式渲染内容
...
- **自动审查 (Auto Review)**: AI 回复完成后，检测修改的文件，用独立 CLI 进程启动只读审查。审查通过返回 `LGTM`，发现问题弹�?Notice。Settings UI 提供开关（`ReviewLoop.ts`, `KiloCodeView.ts`�?
- **技能安装系�?*: `SkillCatalog.installSkill()` 将内置技能模板写�?vault `.kilo/skills/{name}/SKILL.md`，`isSkillInstalled()` 检查安装状�?
- **i18n 条目**: 新增 en/zh �?@mention 相关的翻译键

### Changed

- **KiloCodeView 架构重构**:
  - 移除 `InputController`，改�?Tab 持有 runtime
  - `getOrCreateRuntime()` 优先检�?activeTab.runtime，再检�?`plugin.warmupRuntimeRef`
  - `restartRuntime()` 改为 stop 当前进程 �?置空 tab.runtime �?下一次发送自动重新创�?
  - `handleCancel()` 直接调用 `activeTab.runtime.cancel()`
  - `handleModelSwitch()` 使用 `this.app` �?Modal 基类代替动态导�?
- **TabManager**: `closeTab()` 改为异步 `Promise<boolean>`，关闭前�?dispose 对应�?runtime
- **KiloCodeChatRuntime**:
  - 所�?yield chunk 改通过 `emit()` 方法写入 EventBuffer
  - `sendMessage()` 自动注入技能上下文�?`QUESTION_PROTOCOL`
  - `stop()` 清理 idleTimer、EventBuffer、httpAgent
  - 新增 `KiloCodeTiming` 日志输出 prompt 延迟

### Fixed

- **跨标签切换流式丢�?*: EventBuffer 恢复机制在切换标签时重建流式渲染内容
- **运行时泄�?*: 标签关闭时未清理对应 CLI 进程，现通过 `Tab.disposeRuntime()` 确保 stop
- **CLI 进程残留**: `onClose()` 调用 `tabManager.disposeAllRuntimes()` 统一清理所有标签进�?

### Docs

- **设计文档**: 新增 `docs/superpowers/specs/2026-05-26-mention-design.md` �?`2026-05-26-command-submenu-design.md`

## [0.8.0] - 2026-05-26

### Added

- **@mention 功能**: 将骨架代�?MentionService + MentionDropdown 真正接入聊天界面。工具栏 @ 按钮和输入框 `@` 字符都可触发搜索下拉，支持文�?文件夹（递归�?MCP Server/Subagent 搜索，前缀匹配优先（无结果回退子串匹配），键盘导航（↑↓EnterEsc），i18n 多语言，CSS 样式渲染。单元测试覆�?@Parser�?3 用例）和 MentionService�?5 用例），全量测试无回归�?

### Fixed

- **CORS 绕过**: `@kilocode/sdk/client` 使用浏览�?`fetch()`，在 Obsidian Electron renderer 中因 `app://obsidian.md` origin 访问 `http://127.0.0.1` �?CORS 阻止。新增基�?Node.js `http` 模块�?`nodeFetch` 函数替代全局 `fetch()`，传递给 `createKiloClient` 绕过 CORS。响应头 `Content-Type: text/event-stream` 自动识别�?SSE 流式响应（ReadableStream），其余请求缓冲完整响应�?
- **CI typecheck 失败**: 移除 `createKiloServer` 调用中不存在�?`cors` 属性（`@kilocode/sdk` �?`ServerOptions` 类型无此字段）；修复 `KiloCodeView.ts` �?`adapter.getBasePath()` 调用缺少 `FileSystemAdapter` 类型守卫的问题�?
- **CI 测试失败**: BinaryManager 测试使用不存在的路径导致超时，改用真实临时文件；KiloCodeChatRuntime 测试�?mock 未更新以匹配移除 SSE 订阅后的新架构，补上 `parts` 数据�?
- **SSE 永久挂起**: 移除 `sendMessage()` 中对 `/global/event` SSE 端点的订阅。经 HTTP 测试验证 kilo serve v7.3.1 不通过 SSE 发送事件，所有响应数据通过 `POST /session/{id}/message` 同步返回。改为直接解�?prompt 响应体的 `parts` 数组�?
- **SDK SSE 客户端不传递自定义 fetch**: `serverSentEvents.gen.js` �?`createSseClient` 使用全局 `fetch` 而非 `options.fetch`。已补丁修复�?
- **vault 路径未传递给 CLI**（三层修复）:
  - `KiloCodeView.ts`: 使用�?`app.vault.getRoot().path`（返回虚拟路�?`/`），改为 `app.vault.adapter.getBasePath()`（返回磁盘绝对路径）�?
  - `KiloCodeChatRuntime.ts.ensureServer()`: `createKiloClient` 未传�?`directory` 参数。修复：添加 `directory: vaultPath` 配置项�?
  - `KiloCodeChatRuntime.ts.sendMessage()`: 预热场景下客户端�?vault 路径已知前已创建。修复：支持动态更新客户端配置（`applyVaultPathToClient`），通过 `setConfig` 注入 `x-kilo-directory` 请求头�?

### Changed

- **重构通信�?*: `KiloCodeChatRuntime` 废弃 `kilo run <message>` 子进程模式，改用 `@kilocode/sdk` �?`createKiloServer` / `createKiloClient` API，大幅精简代码�?85 �?�?~400 行）
- **esbuild 配置**: 新增 `kilocode-sdk-resolve` 插件�?ESM-only �?`@kilocode/sdk` 子路径映射到 `dist/*.js`，使 `format: "cjs"` 下能正确打包。新�?`platform: "node"`；tsconfig �?`moduleResolution` 改为 `bundler`；jest 添加 `@kilocode/sdk` 模块映射
- **二进制检测多阶段策略**: `BinaryManager.getBinaryPath()` 按优先级执行 5 阶段检测：手动路径 �?插件目录 �?系统 PATH �?全局 npm �?npm 下载。新�?`DetectionResult` 接口�?`autoDetect()` 方法
- **CLI 配置读取器增�?*: `cliConfigReader.ts` 支持多文件名（kilo.jsonc / kilo.json / config.json），新增 JSONC 解析器（注释、尾逗号），移除 apiKey 相关逻辑
- **模型覆盖修复**: 插件不再硬编�?`modelID` 覆盖 CLI 配置。`defaultModel` 默认值改为空字符串，未显式配置时�?CLI 使用自己的默认模型。设置面板模型下拉框新增 "Use CLI default" 选项

### Added

- **文件附件功能**: 新增 `FileAttachmentContext`（`src/features/chat/ui/FileAttachmentContext.ts`），支持通过系统文件选择器附加任意文件（文本文件内容随消息发送给 AI，二进制文件仅标注文件名）。输入区域显示文件预览芯片（名称+大小+删除按钮）。工具栏 "Attach vault file" 按钮图标从占位符 `??` 修复�?📎�?Attach image" 修复�?🖼️；"Current note" 修复�?📝
- **模型选择支持**: `ChatRuntime` 接口新增 `setModel()` / `getModel()` 方法
- **依赖**: 新增 `@kilocode/sdk: ^7.3.1`
- **ARCHITECTURE.md / DEVELOPMENT.md / ROADMAP.md**: 新增架构、开发指南和路线图文�?

### Docs

- **README 修正**: 根据实际代码实现状态更新功能列表和快速参考表。标注斜杠命令、@提及、内联编�?AI 调用、MCP 连接、图片传输为"待实�?待完�?状态；修正 i18n 宣称�?日文/韩文�?改为"中英�?。同步更新英文和中文 README�?

### Removed

- **SSE 事件订阅**: 移除�?`sendMessage()` 中对 `/global/event` �?SSE 流式订阅逻辑。kilo serve v7.3.1 的所有响应数据通过 `POST /session/{id}/message` 同步返回，SSE 端点不产生实际事件�?

### Added

- **ChatState 集中状态管�?*: `src/features/chat/state/ChatState.ts` �?管理流式状态（isStreaming/streamGeneration/cancelRequested）、会话状态（currentConversationId/hasPendingConversationSave）、流式内容缓冲（currentTextContent/currentThinkingContent/toolCalls）。使�?getter/setter + 回调通知模式，支持事件订阅（streamingChange/cancelRequested/conversationChange�?
- **ConversationController 会话生命周期控制**: `src/features/chat/controllers/ConversationController.ts` �?�?KiloCodeView 抽取会话管理逻辑，提�?createNew()、switchTo()、ensureConversation()（懒创建）、save()、restoreConversation()、rewind()、fork()、addMessage()、getConversation() 方法。通过回调注入（onRenderMessages/onClearMessages）避免直接依�?DOM
- **ContentBlock 类型**: `src/core/types/index.ts` 新增 `ContentBlock` 接口�?`contentBlocks` 字段 �?将消息分解为有序块（text/thinking/tool_use），与现�?thinking/toolCalls 字段并存，提供有序渲染能�?
- **代码块优�?*: MessageRenderer �?`enhanceCodeBlocks()` 方法 �?自动�?`<pre>` 代码块添�?`.kilo-code-wrapper` 包裹、语言标签（从 `class="language-xxx"` 提取）和复制按钮（带 "Copied!" 反馈�?
- **欢迎语随机化**: KiloCodeView �?`getRandomPlaceholder()` 方法 �?textarea 占位符从 5 条提示语中随机选择
- **会话标题显示**: ConversationService �?`getConversationTitle()` 轻量查询方法 �?标签栏显示会话标题而非截断�?ID

### Changed

- **KiloCodeView**: 集成 ChatState �?ConversationController �?构造函数实例化新组件并注入回调，onOpen/handleTabClick/handleSend/handleRewind/handleFork/handleCopy/handleNewTab/onClose 全部迁移至使�?ConversationController。删除已废弃�?`loadConversationMessages()` 方法
...
- **MessageRenderer**: `renderMessage()` �?`finalizeMessage()` �?Markdown 渲染后调�?`enhanceCodeBlocks()` 进行代码块后处理
...
- **ConversationService**: `addMessage()` 磁盘写入防抖 �?内存立即更新保证一致性，磁盘写入延迟 300ms 合并，减少流式响应期间的 I/O 次数。新�?`flush()` 方法在视图关闭时强制写入
- **KiloCodeChatRuntime**: SSE chunk 合并 �?`parseEventStream()` 将同�?`read()` 内相邻的 text/thinking chunk 合并�?yield，减�?`for-await` 循环�?UI 回调次数。新�?`mergeAdjacentChunks()` 生成�?
- **MessageRenderer**: `finalizeMessage()` 延迟 Markdown 渲染 �?使用 `requestAnimationFrame` �?Obsidian `MarkdownRenderer.renderMarkdown()` 推迟到下一帧，避免阻塞 UI 线程
- **KiloCodeChatRuntime**: 重写通信�?�?废弃 `kilo run <message>` 子进程模式，改为 `kilo serve` HTTP 模式。`start()` spawn HTTP server 进程，`sendMessage()` 通过 HTTP POST 发送消息并处理 SSE/ndjson 流式响应，`stop()` kill 进程
- **KiloCodeChatRuntime**: HTTP 请求改用 Node.js `http` 模块 �?浏览�?`fetch()` �?Electron renderer 进程中受 CORS 限制（`app://obsidian.md` origin 无法访问 `http://127.0.0.1`），Node.js HTTP 完全绕过此限�?
- **KiloCodeView**: 重构�?claudian 架构 �?DOM 骨架只在 `onOpen()` 创建一次，通过 `updateUI()` 更新内容，解�?textarea 事件监听器丢失和消息 DOM 被销毁的问题

### Fixed

- **Thinking/Reasoning 文本与正常回答分�?* �?DeepSeek R1 等模型的推理过程（`parts[].type === "thinking"`）不再混入回答显示，通过 `extractThinkingAndText()` �?`type` 字段区分
- **流式响应实时渲染** �?用户发送消息后可看到逐步生成的文本，替代之前�?等待完成后一次性渲�?
- **streamGeneration 冲突保护** �?快速连发消息时旧流不会覆盖新流，通过 `TabState.streamGeneration` 代数匹配机制实现
- **KiloCodeView**: 修复无法发送第二条消息 �?根因�?`render()` 每次调用 `container.empty()` 销�?DOM，导�?textarea �?`registerDomEvent` 事件监听器丢失。现�?textarea 和所有事件监听器只注册一�?
- **KiloCodeView**: 修复切换会话时消息消�?�?根因�?`render()` 销毁消�?DOM �?`renderConversationMessages` 未正确调用。现在通过 `handleTabClick()` �?`loadConversationMessages()` 正确加载消息
- **KiloCodeView**: 修复重启 Obsidian 后首个会话无法发�?�?根因�?`render()` �?`onOpen()` 时被调用但会话尚未初始化。现�?`buildLayout()` 只创建骨架，消息加载异步进行
- **KiloCodeView**: 修复流式响应串台到其他标签页 �?添加 `senderTabId` 跟踪发送者标签，流式进行中阻止切换标签，渲染前校验当前标签是否为发送�?
- **KiloCodeView**: 修复聊天发送消息时 "Runtime not started" 错误 �?`getOrCreateRuntime()` 改为 async 等待 `start()` 完成后再返回 runtime
- **CORS**: 修复 Obsidian 插件无法访问 `kilo serve` HTTP API �?CORS 策略错误

### Added

- **StreamChunkType**: 新增 `'thinking'` 类型，支�?thinking/reasoning 文本与普通文本分�?
- **Message**: 新增 `thinking?: string` 字段，持久化 thinking/reasoning 内容
- **TabState**: 新增 `streamGeneration: number` 字段�?`bumpStreamGeneration()` 方法，用于流式冲突保�?
- **KiloCodeChatRuntime**: `extractThinkingAndText()` 方法 �?递归遍历 JSON 结构，按 `parts[].type` 字段区分 thinking �?text
- **StreamController**: `onThinking` 回调 + `generation` 参数 �?支持 thinking chunk 处理�?streamGeneration 冲突保护
- **MessageRenderer**: 流式增量渲染 �?`addAssistantMessage()` 创建空容器、`appendText()` 增量文本追加（textContent 避免高频 Markdown 渲染）、`appendThinking()` 创建/更新 thinking block、`finalizeMessage()` 流结束后最�?Markdown 渲染
- **MessageRenderer**: thinking block 折叠显示 �?流式阶段 `<details>` 展开显示、历史消�?`<details>` 折叠显示并标注字符数
- **SettingsTab**: 新增 API Configuration 区域 �?API Key（密码输入框）和 Base URL 配置�?
- **KiloCodeChatRuntime**: 环境变量注入 �?`apiKey` �?`KILO_API_KEY`，`baseUrl` �?`KILO_BASE_URL`，`vaultPath` �?`cwd`
- **registration**: `createKilocodeRegistration()` 接受 settings getter，确�?runtime 使用最新的用户配置而非默认空�?
...
- **README.md**: 全面细化文档内容 �?新增输入工具栏、对话管理、图片附件、权限系统使用说明；架构章节重写为完整目录树+数据�?设计决策；新�?Security Model、Testing、i18n、CI/CD 独立章节；扩展故障排除与路线�?
- **README_CN.md**: 与英文版保持同步的中文细�?
