// src/providers/kilocode/runtime/SkillCatalog.ts
//
// 技能编目系统（T3.3）：
// 提供预定义的官方技能列表和安装功能，让用户通过简单命令添加常用技能。
//
// 技能模板存储在插件中，安装时复制到 .kilo/skills/<name>/SKILL.md。
// 已存在的技能不会被覆盖（用户可能已自定义修改）。
...
/**
 * 安装技能到 vault 的 .kilo/skills/<name>/ 目录。
 *
 * @param vaultPath vault 根目录路径
 * @param skillName 技能名称（对应 catalog 中的 name）
 * @returns 安装结果
 */
